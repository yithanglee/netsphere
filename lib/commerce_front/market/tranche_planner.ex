defmodule CommerceFront.Market.TranchePlanner do
    ...
    def next_tranche_price(%Asset{} = asset) do
      ...
      case asset.price_increment_type do
        :fixed_amount -> Decimal.add(prev_price, asset.price_increment_value)
        :percent -> Decimal.mult(prev_price, Decimal.add(Decimal.new("1"), asset.price_increment_value))
      end
    end
    ...
    def create_next_tranche(asset_id, quantity) do
      ...
      next_seq =
        (Repo.one(from(t in AssetTranche, where: t.asset_id == ^asset_id, select: max(t.seq))) || 0) + 1
